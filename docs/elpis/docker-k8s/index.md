...
<div class='original-text'>DevOps 这块我主要做了容器化改造和CI/CD 流水线建设。之前项目部署很原始，要手动打包、上传服务器、重启服务，经常出错，部署一次要半天。而且环境不一致，本地能跑线上出问题，排查很麻烦。我推动做了三件事：第一是容器化改造。用 Docker 把前后端应用都容器化，写了 Dockerfile 定义镜像构建流程。好处是环境一致，本地、测试、生产用同一个镜像，不会有环境差异问题。而且镜像可以版本管理，回滚很方便。第二是CI/CD 流水线。基于 GitLab CI，配置了自动化流水线。开发提交代码到 Git，自动触发构建：先跑单元测试，通过了构建 Docker 镜像，推送到镜像仓库，再自动部署到对应环境。测试环境每次提交都自动部署，生产环境需要手动审批。第三是K8s 编排部署。用 Kubernetes 管理容器，写了 Deployment、Service、Ingress 配置。支持多副本部署、滚动更新、健康检查、自动重启。配置了水平扩缩容，访问量大时自动加实例，流量低时自动减少，节省资源。效果很明显：部署时间从半天降到10分钟；部署成功率从70%提升到95%；出问题可以一键回滚，从半小时降到1分钟；环境一致性问题基本没有了；而且支持灰度发布，新版本先给10%用户，没问题再全量，降低了风险。</div>
